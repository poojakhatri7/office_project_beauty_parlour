  ...
  if (isset($_POST['services']) && !empty($_POST['services'])) {
    ...
    for ($i = 0; $i < $N; $i++) {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            {
              echo"<script> alert('Your booking has been placed') </script>";
            }
...
//  $result = mysqli_query($conn, $sql);
//   if (mysqli_num_rows($result) > 0) { // No existing order, insert a new one
// $sql_insert = "INSERT INTO orders (appointment_id, totalPrice, discount,billing_number,created_at) VALUES ({$appointment_id}, {$totalPrice}, {$discount},{$billing_number},CURTIME())";
...
// if (mysqli_num_rows($result) == 0) { // No existing order, insert a new one
//     // $order_sql = "INSERT INTO orders (appointment_id, billing_number, total_amount, order_date)
//     //               VALUES ('$appointment_id', '$billing_number', '$total_amount', NOW())";
//                    $sql_insert = "INSERT INTO orders (appointment_id, totalPrice, discount) VALUES ({$appointment_id}, {$totalPrice}, {$discount})";
//                    $order_sql   = mysqli_query($conn, $sql_insert);
//     mysqli_query($conn, $order_sql);
// } else {
//     echo "Order already exists for this appointment.";
// }
}
    ...
    //             $insert_sql = "INSERT INTO tb_selected_services (appointment_id, service_name, service_price, billing_number) 
    //                            VALUES ('$appointment_id', '$service_name', '$service_price', '$billing_number')";
    //             if (!mysqli_query($conn, $insert_sql)) {
    //                 die("Error inserting service: " . mysqli_error($conn));
    //             }
    //         } else {
    //             die("Service '$service_name' not found in the database.");
    //         }
    //     }
    
    //     echo "Billing Number: $billing_number assigned successfully!";
    // } else {
...
<button type="submit" name="submit" class="btn" style="background-color:rgb(51, 139, 139); font-weight: 500; font-size: 16px;  padding: 7px 20px; ">Book</button>
