...
.search_appointment{
  ...
  <body>
    ...
    <div class="card-header">
                <h3 class="card-title" style='font-weight: 500;'>Appointment Details Search By Mobile Number</h3>
              </div>
...
<?php 
  ...
  if ($appointment_id) { // Ensure appointment_id is available
      $sql1 = "SELECT DISTINCT appointment_id FROM tb_selected_services WHERE appointment_id = {$appointment_id}";
...
?>
              <td>  
                  <a href='/beauty_parlour_management_system/admin2/invoice_details2.php?appointment_id=<?php echo $row1["appointment_id"]; ?>'>
                      <button class='btn' style='background-color:rgb(23, 162, 184); color: white; border: none; padding: 5px 10px; cursor: pointer;'>
